...
QString ParseRequestWorker::http_attribute_encode(QString attribute_name, QString input) {
    ...
    for (int i = 0; i < input_c.length(); i++) {
        c = input_c.at(i);
        if (c == '\\' || c == '/' || c == '\0' || c < ' ' || c > '~') need_utf_encoding = true; // ignore and request utf-8 version
        else if (c == '"') result += "\\\"";
        else result += c;
    }

    if (result.length() == 0) need_utf_encoding = true;

    if (!need_utf_encoding) return QString("%1=\"%2\"").arg(attribute_name, result); //return simple version

    QString result_utf8 = "";
    for (int i = 0; i < input_c.length(); i++) {
        c = input_c.at(i);
        if ((c >= '0' && c <= '9') || (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z')) result_utf8 += c;
        else result_utf8 += "%" + QString::number(static_cast<unsigned char>(input_c.at(i)), 16).toUpper();
    }
...
void ParseRequestWorker::execute(ParseRequestInput *input) {
    ...
    if (input->files.length() > 0) input->var_layout = MULTIPART;

    if (input->var_layout == NOT_SET) input->var_layout = (input->http_method == "GET" || input->http_method == "HEAD") ? ADDRESS : URL_ENCODED;
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        if (input->vars.count() > 0) {
            ...
            foreach (QString key, input->vars.keys()) {
                if (!first) request_content.append("&");
                first = false;
        ...
        for (QList<ParseRequestInputFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
            ...
            if (!file.open(QIODevice::ReadOnly)) continue; //silent abort for the current file

            // ensure filename for the request
            if (file_info->request_filename == NULL || file_info->request_filename.isEmpty()) {
                file_info->request_filename = fi.fileName();
                if (file_info->request_filename.isEmpty()) file_info->request_filename = "file";
            }
    ...
    else if (input->http_method == "PUT") manager->put(request, request_content);
    else if (input->http_method == "HEAD") manager->head(request);
    else if (input->http_method == "DELETE") manager->deleteResource(request);
    else {
